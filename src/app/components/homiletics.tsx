...
import { homileticsService } from '@/services/homiletics.service';
...
export function Homiletics() {
  ...
  const { session, canAccessStage, setStage } = useStudySession();

  useEffect(() => {
    const loadHomiletics = async () => {
      try {
        const data = await homileticsService.get();
          ...
          <div className="px-10 py-8">
            ...
            <div className="mb-6 pb-4 border-b border-[var(--divider)]">
              ...
              <h1 className="text-xl tracking-tight text-[var(--graphite)] mb-1">
                {session ? `${session.book} ${session.chapter}` : 'Romans 3:21-26'}
              </h1>
          ...
          <div className="px-10 py-8">
            ...
            <div className="mb-6">
              ...
              <div className="mt-3 flex items-center gap-2">
                <button
                  onClick={() => setStage('homiletics')}
                  disabled={!canAccessStage('canonical-correlation')}
                  className="px-3 py-1 text-xs rounded border border-[var(--divider)] disabled:opacity-50"
                >
                  Enter Homiletics Stage
                </button>
                <button
                  disabled={session?.stage !== 'homiletics'}
                  className="px-3 py-1 text-xs rounded bg-[var(--deep-navy)] text-white disabled:opacity-50"
                >
                  Export Sermon
                </button>
              </div>
