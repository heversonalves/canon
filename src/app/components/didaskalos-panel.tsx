...
export function DidaskaloPanel({ context, suggestions, currentSource = 'general' }: DidaskalosPanelProps) {
  ...
  useEffect(() => {
    const loadNotes = async () => {
      try {
        const data = await api.get<Note[]>(`/api/notes?source=${currentSource}`);
        setNotes(data.map((note) => ({ ...note, timestamp: new Date(note.timestamp) })));
  ...
  const handleAddNote = () => {
    if (newNoteContent.trim()) {
      ...
      api.post('/api/notes', {
        ...newNote,
        pinned: newNote.isPinned,
        created_at: newNote.timestamp.toISOString()
      }).catch(() => undefined);
      setNewNoteContent('');
  ...
  const handleDeleteNote = (id: string) => {
    ...
    api.delete(`/api/notes/${id}`).catch(() => undefined);
  };
