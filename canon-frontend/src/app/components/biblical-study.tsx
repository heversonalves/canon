...
const mockChapters: { [key: string]: any } = {
  'Romans-3': {
    verses: [
      { 
        num: 21, 
        translation: "Mas agora se manifestou, sem a lei, a justiça de Deus, tendo o testemunho da lei e dos profetas;",
      ...
      { 
        num: 22, 
        translation: "Isto é, a justiça de Deus pela fé em Jesus Cristo para todos e sobre todos os que creem; porque não há diferença.",
      ...
      { 
        num: 23, 
        translation: "Porque todos pecaram e destituídos estão da glória de Deus;",
      ...
      { 
        num: 24, 
        translation: "Sendo justificados gratuitamente pela sua graça, pela redenção que há em Cristo Jesus;",
      ...
      { 
        num: 25, 
        translation: "Ao qual Deus propôs para propiciação pela fé no seu sangue, para demonstrar a sua justiça pela remissão dos pecados dantes cometidos, sob a paciência de Deus;",
      ...
      { 
        num: 26, 
        translation: "Para demonstração da sua justiça neste tempo presente, para que ele seja justo e justificador daquele que tem fé em Jesus.",
  ...
  'Genesis-1': {
    verses: [
      {
        num: 1,
        translation: "No princípio criou Deus os céus e a terra.",
      ...
      {
        num: 2,
        translation: "E a terra era sem forma e vazia; e havia trevas sobre a face do abismo; e o Espírito de Deus se movia sobre a face das águas.",
      ...
      {
        num: 3,
        translation: "E disse Deus: Haja luz; e houve luz.",
...
export function BiblicalStudy() {
  ...
  const applyHighlight = (color: string) => {
    if (selectedText.text && selectedText.verseNum) {
      ...
      if (verse) {
        const startIndex = verse.translation.indexOf(selectedText.text);
        if (startIndex !== -1) {
  ...
  const renderHighlightedText = (verse: any) => {
    ...
    sortedHighlights.forEach((highlight, idx) => {
      if (highlight.startIndex > lastIndex) {
        parts.push(
          <span key={`text-${idx}`}>{verse.translation.substring(lastIndex, highlight.startIndex)}</span>
        );
      }
    ...
    if (lastIndex < verse.translation.length) {
      parts.push(
        <span key="text-end">{verse.translation.substring(lastIndex)}</span>
      );
    }
